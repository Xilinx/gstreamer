...
{
  int i;
  guint16 *restrict dy = GET_LINE (y);
  guint16 Y;
  const guint16 *restrict s = src;
...
/* Same as PACK_P010_10LE except suv/duv setting, like NV16/NV12 */
    ...
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    Y = GST_READ_UINT16_LE (sy + i) << 6;
    U = GST_READ_UINT16_LE (suv + (i & ~1)) << 6;
    V = GST_READ_UINT16_LE (suv + (i & ~1) + 1) << 6;
    ...
    gint y, gint width)
{
  int i;
  guint16 *restrict dy = GET_Y_LINE (y);
  guint16 *restrict duv = GET_U_LINE (y);
...
#define PACK_P212_12LE GST_VIDEO_FORMAT_AYUV64, unpack_P212_12LE, 1, pack_P212_12LE
static void
unpack_P212_12LE (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
  ...
  for (i = 0; i < width; i++) {
    Y = GST_READ_UINT16_LE (sy + i) << 4;
    U = GST_READ_UINT16_LE (suv + (i & ~1)) << 4;
    V = GST_READ_UINT16_LE (suv + (i & ~1) + 1) << 4;
...
static void
pack_P212_12LE (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
...
float_to_bf16 (float f)
{
  ...
  u.f = f;
  return (guint16) (u.u >> 16);
}
static inline float
fp16_to_float (guint16 fp16)
{
  union {
    guint32 u;
    float f;
  } u;
  guint32 sign = (fp16 & 0x8000) << 16;
  guint32 exp = (fp16 & 0x7C00) >> 10;
  guint32 mant = (fp16 & 0x03FF);
  ...
  if (exp == 0) {
    ...
    /* Normalized */
    u.u = sign | ((exp + (127 - 15)) << 23) | (mant << 13);
  }
  return u.f;
}
...
float_to_fp16 (float f)
{
  ...
  u.f = f;
  guint32 sign = (u.u & 0x80000000) >> 16;
  gint32 exp = ((u.u & 0x7F800000) >> 23) - 127 + 15;
  guint32 mant = (u.u & 0x007FFFFF);
  ...
  if (exp <= 0) {
    ...
    return (guint16) (sign | (mant >> 13));
  } else if (exp >= 0x1F) {
    /* Overflow, return infinity */
    return (guint16) (sign | 0x7C00);
  }
  /* Normalized */
  return (guint16) (sign | (exp << 10) | (mant >> 13));
}

/* GRAY_FP16 (16-bit grayscale FP16) */
#define PACK_GRAY_FP16 GST_VIDEO_FORMAT_AYUV64, unpack_GRAY_FP16, 1, pack_GRAY_FP16
static void
unpack_GRAY_FP16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  const guint16 *restrict s = (const guint16 *) GET_LINE (y);
  guint8 *restrict d = dest;
  gint i;
  ...
  for (i = 0; i < width; i++) {
    ...
    d[i * 4 + 0] = 0xff;
    d[i * 4 + 1] = CLAMP ((gint) (v * 255.0f + 0.5f), 0, 255);
    d[i * 4 + 2] = 0x80;
    d[i * 4 + 3] = 0x80;
  }
}
...
static void
pack_GRAY_FP16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
  guint16 *restrict d = (guint16 *) GET_LINE (y);
  const guint8 *restrict s = src;
  gint i;

  for (i = 0; i < width; i++) {
    d[i] = float_to_fp16 (s[i * 4 + 1] / 255.0f);
  }
}

/* GRAY_BF16 (16-bit grayscale BFloat16) */
#define PACK_GRAY_BF16 GST_VIDEO_FORMAT_AYUV64, unpack_GRAY_BF16, 1, pack_GRAY_BF16
static void
unpack_GRAY_BF16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  const guint16 *restrict s = (const guint16 *) GET_LINE (y);
...
static void
pack_GRAY_BF16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    d[i] = float_to_bf16 (s[i * 4 + 1] / 255.0f);
  }
}

/* GRAY_FLOAT (32-bit grayscale FLOAT) */
#define PACK_GRAY_FLOAT GST_VIDEO_FORMAT_AYUV64, unpack_GRAY_FLOAT, 1, pack_GRAY_FLOAT
static void
unpack_GRAY_FLOAT (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  const float *restrict s = (const float *) GET_LINE (y);
  guint8 *restrict d = dest;
  gint i;
...
static void
pack_GRAY_FLOAT (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
  float *restrict d = (float *) GET_LINE (y);
  const guint8 *restrict s = src;
  gint i;
...
{
  int i;
  const guint16 *restrict s = (const guint16 *) (GET_LINE (y)) + x * 4;
  guint8 *restrict d = dest;

  for (i = 0; i < width; i++) {
    float r = bf16_to_float (s[0]);
    float g = bf16_to_float (s[1]);
    float b = bf16_to_float (s[2]);
    float a = bf16_to_float (s[3]);
    d[0] = CLAMP ((gint) (a * 255.0f + 0.5f), 0, 255);
    d[1] = CLAMP ((gint) (r * 255.0f + 0.5f), 0, 255);
    d[2] = CLAMP ((gint) (g * 255.0f + 0.5f), 0, 255);
    d[3] = CLAMP ((gint) (b * 255.0f + 0.5f), 0, 255);
    s += 4;
    d += 4;
  }
}
    ...
    gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    d[0] = float_to_bf16 (s[1] / 255.0f);
    d[1] = float_to_bf16 (s[2] / 255.0f);
    d[2] = float_to_bf16 (s[3] / 255.0f);
    d[3] = float_to_bf16 (s[0] / 255.0f);
    d += 4;
    s += 4;
  }
}
    ...
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    float r = fp16_to_float (s[0]);
    float g = fp16_to_float (s[1]);
    float b = fp16_to_float (s[2]);
    float a = fp16_to_float (s[3]);
    d[0] = CLAMP ((gint) (a * 255.0f + 0.5f), 0, 255);
    d[1] = CLAMP ((gint) (r * 255.0f + 0.5f), 0, 255);
    d[2] = CLAMP ((gint) (g * 255.0f + 0.5f), 0, 255);
    d[3] = CLAMP ((gint) (b * 255.0f + 0.5f), 0, 255);
    ...
    gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    d[0] = float_to_fp16 (s[1] / 255.0f);
    d[1] = float_to_fp16 (s[2] / 255.0f);
    d[2] = float_to_fp16 (s[3] / 255.0f);
    d[3] = float_to_fp16 (s[0] / 255.0f);
    d += 4;
    s += 4;
  }
}
...
{
  int i;
  const float *restrict s = (const float *) (GET_LINE (y)) + x * 4;
  guint16 *restrict d = dest;
  ...
  for (i = 0; i < width; i++) {
    ...
    float b = s[2];
    float a = s[3];
    d[0] = CLAMP ((gint) (a * 65535.0f + 0.5f), 0, 65535);
    d[1] = CLAMP ((gint) (r * 65535.0f + 0.5f), 0, 65535);
    d[2] = CLAMP ((gint) (g * 65535.0f + 0.5f), 0, 65535);
    d[3] = CLAMP ((gint) (b * 65535.0f + 0.5f), 0, 65535);
    s += 4;
    d += 4;
  }
}
...
static void
pack_RGBA8_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
...
#define PACK_RGBA_FP16_C8 GST_VIDEO_FORMAT_ARGB, unpack_RGBA_FP16_C8, 1, pack_RGBA_FP16_C8
static void
unpack_RGBA_FP16_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
  const guint16 *restrict s = (const guint16 *) (GET_LINE (y)) + x * 8;
  guint8 *restrict d = dest;
...
static void
pack_RGBA_FP16_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
...
#define PACK_RGBA_BF16_C8 GST_VIDEO_FORMAT_ARGB, unpack_RGBA_BF16_C8, 1, pack_RGBA_BF16_C8
static void
unpack_RGBA_BF16_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
...
static void
pack_RGBA_BF16_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
...
#define PACK_RGBA_FP32_C8 GST_VIDEO_FORMAT_ARGB, unpack_RGBA_FP32_C8, 1, pack_RGBA_FP32_C8
static void
unpack_RGBA_FP32_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
  const float *restrict s = (const float *) (GET_LINE (y)) + x * 8;
  guint8 *restrict d = dest;
...
static void
pack_RGBA_FP32_C8 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
...
#define PACK_BGR_BF16 GST_VIDEO_FORMAT_ARGB, unpack_BGR_BF16, 1, pack_BGR_BF16
static void
unpack_BGR_BF16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
...
static void
pack_BGR_BF16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    d[0] = float_to_bf16 (s[i * 4 + 3] / 255.0f);
    d[1] = float_to_bf16 (s[i * 4 + 2] / 255.0f);
    d[2] = float_to_bf16 (s[i * 4 + 1] / 255.0f);
    d[3] = float_to_bf16 (s[i * 4 + 0] / 255.0f); /* A */
    d += 4;
  }
}

#define PACK_BGR_FP16 GST_VIDEO_FORMAT_ARGB, unpack_BGR_FP16, 1, pack_BGR_FP16
static void
unpack_BGR_FP16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
...
static void
pack_BGR_FP16 (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    d[0] = float_to_fp16 (s[i * 4 + 3] / 255.0f);
    d[1] = float_to_fp16 (s[i * 4 + 2] / 255.0f);
    d[2] = float_to_fp16 (s[i * 4 + 1] / 255.0f);
    d[3] = float_to_fp16 (s[i * 4 + 0] / 255.0f); /* A */
    d += 4;
  }
}

#define PACK_BGR_FLOAT GST_VIDEO_FORMAT_ARGB, unpack_BGR_FLOAT, 1, pack_BGR_FLOAT
static void
unpack_BGR_FLOAT (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    gpointer dest, const gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], gint x, gint y, gint width)
{
  int i;
...
static void
pack_BGR_FLOAT (const GstVideoFormatInfo * info, GstVideoPackFlags flags,
    const gpointer src, gint sstride, gpointer data[GST_VIDEO_MAX_PLANES],
    const gint stride[GST_VIDEO_MAX_PLANES], GstVideoChromaSite chroma_site,
    gint y, gint width)
{
  ...
  for (i = 0; i < width; i++) {
    d[0] = s[i * 4 + 3] / 255.0f; /* A */
    d[1] = s[i * 4 + 2] / 255.0f; /* B */
    d[2] = s[i * 4 + 1] / 255.0f; /* G */
    d[3] = s[i * 4 + 0] / 255.0f; /* R */
    d += 4;
    s += 4;
  }
}
...
#define PSTR32323232      { 32, 32, 32, 32 }
...
#define MAKE_GRAY_AI_LAYOUT(name, desc, depth, pstride, plane, offs, sub, pack) \
 { 0x00000000, {GST_VIDEO_FORMAT_ ##name, G_STRINGIFY(name), desc, GST_VIDEO_FORMAT_FLAG_GRAY | GST_VIDEO_FORMAT_FLAG_AI_LAYOUT, depth, pstride, plane, offs, sub, pack } }
...
#define MAKE_RGBA_AI_LAYOUT(name, desc, depth, pstride, plane, offs, sub, pack) \
 { 0x00000000, {GST_VIDEO_FORMAT_ ##name, G_STRINGIFY(name), desc, GST_VIDEO_FORMAT_FLAG_RGB | GST_VIDEO_FORMAT_FLAG_ALPHA | GST_VIDEO_FORMAT_FLAG_AI_LAYOUT, depth, pstride, plane, offs, sub, pack } }
#define MAKE_RGB_AI_LAYOUT(name, desc, depth, pstride, plane, offs, sub, pack) \
 { 0x00000000, {GST_VIDEO_FORMAT_ ##name, G_STRINGIFY(name), desc, GST_VIDEO_FORMAT_FLAG_RGB | GST_VIDEO_FORMAT_FLAG_AI_LAYOUT, depth, pstride, plane, offs, sub, pack } }
...
static const VideoFormat formats[] = {
  ...
  MAKE_YUV_LE_FORMAT (P010_10LE, "raw video", 0x00000000, DPTH10_10_10,
      PSTR244, PLANE011, OFFS001, SUB420, PACK_P010_10LE),
  MAKE_YUV_LE_FORMAT (P210_10LE, "raw video", 0x00000000, DPTH10_10_10,
      PSTR244, PLANE011, OFFS001, SUB422, PACK_P210_10LE),
  MAKE_YUV_LE_FORMAT (P212_12LE, "raw video", 0x00000000, DPTH12_12_12,
      PSTR244, PLANE011, OFFS001, SUB422, PACK_P212_12LE),
  ...
  MAKE_YUV_LE_FORMAT (P012_LE, "raw video", 0x00000000, DPTH12_12_12,
  ...
  MAKE_YUV_C_LE_FORMAT (Y444_10LE32, "raw video",
      GST_MAKE_FOURCC ('X', '4', '0', '3'), DPTH10_10_10, PSTR0, PLANE012,
      OFFS0, SUB444, PACK_Y444_10LE32),
  ...
  MAKE_RGB_FORMAT (BGRP, "raw video", DPTH888, PSTR111, PLANE210, OFFS0, SUB444,
      PACK_BGRP),
  MAKE_GRAY_AI_LAYOUT (GRAY_BF16, "raw video", DPTH16, PSTR2, PLANE0, OFFS0, SUB4,
     PACK_GRAY_BF16),
  MAKE_GRAY_AI_LAYOUT (GRAY_FP16, "raw video", DPTH16, PSTR2, PLANE0, OFFS0, SUB4,
     PACK_GRAY_FP16),
  MAKE_GRAY_AI_LAYOUT (GRAY_FLOAT, "raw video", DPTH32, PSTR4, PLANE0, OFFS0, SUB4,
     PACK_GRAY_FLOAT),
  MAKE_RGBA_AI_LAYOUT (RGBA8_C4, "raw video", DPTH8888, PSTR4444,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA8_C4),
  MAKE_RGBA_AI_LAYOUT (RGBX8_C4, "raw video", DPTH8888, PSTR4444,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA8_C4),
  MAKE_RGBA_AI_LAYOUT (RGBA_BF16_C4, "raw video", DPTH16_16_16_16, PSTR8888,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_BF16_C4),
  MAKE_RGBA_AI_LAYOUT (RGBX_BF16_C4, "raw video", DPTH16_16_16_16, PSTR8888,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_BF16_C4),
  MAKE_RGBA_AI_LAYOUT (RGBA_FP16_C4, "raw video", DPTH16_16_16_16, PSTR8888,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FP16_C4),
  MAKE_RGBA_AI_LAYOUT (RGBX_FP16_C4, "raw video", DPTH16_16_16_16, PSTR8888,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FP16_C4),
  MAKE_RGBA_AI_LAYOUT (RGBA_FLOAT_C4, "raw video", DPTH32_32_32_32, PSTR16161616,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FLOAT_C4),
  MAKE_RGBA_AI_LAYOUT (RGBX_FLOAT_C4, "raw video", DPTH32_32_32_32, PSTR16161616,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FLOAT_C4),
  MAKE_RGBA_AI_LAYOUT (RGBA8_C8, "raw video", DPTH8888, PSTR8888,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA8_C8),
  MAKE_RGBA_AI_LAYOUT (RGBA_FP16_C8, "raw video", DPTH16_16_16_16, PSTR16161616,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FP16_C8),
  MAKE_RGBA_AI_LAYOUT (RGBA_BF16_C8, "raw video", DPTH16_16_16_16, PSTR16161616,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_BF16_C8),
  MAKE_RGBA_AI_LAYOUT (RGBA_FLOAT_C8, "raw video", DPTH32_32_32_32, PSTR32323232,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FP32_C8),
  MAKE_RGBA_AI_LAYOUT (RGBX8_C8, "raw video", DPTH8888, PSTR8888,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA8_C8),
  MAKE_RGBA_AI_LAYOUT (RGBX_FP16_C8, "raw video", DPTH16_16_16_16, PSTR16161616,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FP16_C8),
  MAKE_RGBA_AI_LAYOUT (RGBX_BF16_C8, "raw video", DPTH16_16_16_16, PSTR16161616,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_BF16_C8),
  MAKE_RGBA_AI_LAYOUT (RGBX_FLOAT_C8, "raw video", DPTH32_32_32_32, PSTR32323232,
      PLANE0, OFFS0123, SUB4444, PACK_RGBA_FP32_C8),

  MAKE_RGB_AI_LAYOUT (BGR_BF16, "raw video", DPTH16_16_16, PSTR666, PLANE0, OFFS210,
      SUB444, PACK_BGR_BF16),
  MAKE_RGB_AI_LAYOUT (BGR_FP16, "raw video", DPTH16_16_16, PSTR666, PLANE0, OFFS210,
      SUB444, PACK_BGR_FP16),
  MAKE_RGB_AI_LAYOUT (BGR_FLOAT, "raw video", DPTH32_32_32, PSTR121212, PLANE0, OFFS210, SUB444,
      PACK_BGR_FLOAT),
  MAKE_RGBA_AI_LAYOUT (BGRA8, "raw video", DPTH8888, PSTR4444, PLANE0, OFFS0123, SUB4444,
      PACK_RGBA8_C4),
  MAKE_RGBA_AI_LAYOUT (BGRA_BF16, "raw video", DPTH16_16_16_16, PSTR8888, PLANE0, OFFS2103,
      SUB4444, PACK_RGBA_BF16_C4),
  MAKE_RGBA_AI_LAYOUT (BGRA_FP16, "raw video", DPTH16_16_16_16, PSTR8888, PLANE0, OFFS2103,
  ...
  MAKE_GRAY_C_LE_FORMAT (GRAY10_LE, "raw video", DPTH10, PSTR2, PLANE0, OFFS0,
      SUB4, PACK_GRAY10_LE),
