...
/* HDR10 Plus Meta implementation */

GType
gst_video_hdr10_plus_meta_api_get_type (void)
{
  ...
  if (g_once_init_enter (&type)) {
    ...
    GType _type = gst_meta_api_type_register ("GstVideoHDR10PlusMetaAPI", tags);
...
static gboolean
gst_video_hdr10_plus_meta_init (GstMeta * meta, gpointer params,
    GstBuffer * buffer)
{
  GstVideoHDR10PlusMeta *emeta = (GstVideoHDR10PlusMeta *) meta;
...
const GstMetaInfo *
gst_video_hdr10_plus_meta_get_info (void)
{
  static const GstMetaInfo *meta_info = NULL;

  if (g_once_init_enter ((GstMetaInfo **) & meta_info)) {
    const GstMetaInfo *mi =
        gst_meta_register (GST_VIDEO_HDR10_PLUS_META_API_TYPE,
        "GstVideoHDR10PlusMeta",
        sizeof (GstVideoHDR10PlusMeta),
        gst_video_hdr10_plus_meta_init,
        NULL,
        NULL);
...
/**
 * gst_buffer_add_video_hdr10_plus_meta:
 * @buffer: a #GstBuffer
 * @data: GstVideoHDR10Plus data
 * Attaches #GstVideoHDR10PlusMeta metadata to @buffer with the given
 * parameters.
 *
 * Returns: (transfer none): the #GstVideoHDR10PlusMeta on @buffer.
 *
 *
 * Since: 1.18
 */
GstVideoHDR10PlusMeta *
gst_buffer_add_video_hdr10_plus_meta (GstBuffer * buffer,
    GstVideoHDR10Plus * data)
{
  GstVideoHDR10PlusMeta *meta;

  g_return_val_if_fail (GST_IS_BUFFER (buffer), NULL);
  g_return_val_if_fail (data != NULL, NULL);

  meta = (GstVideoHDR10PlusMeta *) gst_buffer_add_meta (buffer,
      GST_VIDEO_HDR10_PLUS_META_INFO, NULL);
  g_assert (meta != NULL);
...
/**
 * gst_video_hdr10_plus_init:
 * @linfo: a #GstVideoHDR10Plus
 *
 * Initialize @linfo
 *
 * Since: 1.18
 */
void
gst_video_hdr10_plus_init (GstVideoHDR10Plus * hdrplus)
{
  g_return_if_fail (hdrplus != NULL);

  memset (hdrplus, 0, sizeof (GstVideoHDR10Plus));
}
