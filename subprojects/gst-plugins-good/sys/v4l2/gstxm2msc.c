...
gst_v4l2_probe_and_register (GstPlugin * plugin)
{
  ...
  while (gst_v4l2_iterator_next (it)) {
    ...
    if (gst_v4l2_is_bconvert (sink_caps, src_caps)) {
      gst_v4l2_bconvert_register (plugin, basename, it->device_path,
          sink_caps, src_caps);
    }
    /* else if ( ... etc. */
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    v4l2xm2msc,
    "v4l2xm2msc for xilinx based ISP mimo usecases",
    plugin_init, VERSION, GST_LICENSE, "xm2msc", "www.amd.com")
