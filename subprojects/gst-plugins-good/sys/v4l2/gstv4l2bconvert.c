...
#define V4L2_BCONVERT_QUARK \
	g_quark_from_static_string("gst-v4l2-bconvert-info")
...
typedef struct
{
  ...
  GstCaps *sink_caps;
  GstCaps *src_caps;
} GstV4l2BConvertCData;

#define gst_v4l2_bconvert_parent_class parent_class
G_DEFINE_ABSTRACT_TYPE (GstV4l2BConvert, gst_v4l2_bconvert,
    GST_TYPE_BASE_TRANSFORM);

static void
gst_v4l2_bconvert_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (object);
...
static void
gst_v4l2_bconvert_get_property (GObject * object,
...
static gboolean
gst_v4l2_bconvert_open (GstV4l2BConvert * self)
{
...
static void
gst_v4l2_bconvert_close (GstV4l2BConvert * self)
{
...
static gboolean
gst_v4l2_bconvert_stop (GstV4l2BConvert * trans)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (trans);
...
static gboolean
gst_v4l2_bconvert_set_caps (GstV4l2BConvert * trans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstV4l2Error error = GST_V4L2_ERROR_INIT;
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (trans);
...
static gboolean
gst_v4l2_bconvert_query (GstV4l2BConvert * trans, GstPadDirection direction,
    GstQuery * query)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (trans);
  gboolean ret = TRUE;
...
static gboolean
gst_v4l2_bconvert_decide_allocation (GstV4l2BConvert * trans, GstQuery * query)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (trans);
  gboolean ret = FALSE;
...
static gboolean
gst_v4l2_bconvert_propose_allocation (GstV4l2BConvert * trans,
...
/* The caps can be bconverted into any other caps.
 * However, we should prefer passthrough, so if passthrough is possible,
 * put it first in the list. */
static GstCaps *
gst_v4l2_bconvert_bconvert_caps (GstV4l2BConvert * btrans,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter)
{
/*
FIXME :: This option can be moved to v4l2transform.c and handled properly to
support this additional feature as part of v4l2convert element.
Subsequent releases plans: reduce this code base and reuse v4l2transform.
*/
  GstCaps *result_caps;
  GstStructure *structure;
  if (direction == GST_PAD_SINK) {
    result_caps = gst_caps_new_empty_simple ("video/x-raw");
    structure = gst_caps_get_structure (caps, 0);
    gst_caps_append_structure (result_caps,
        gst_structure_new ("video/x-raw",
            "format", G_TYPE_STRING, gst_structure_get_value (structure,
                "format"), "width", GST_TYPE_INT_RANGE, 1, G_MAXINT, "height",
            GST_TYPE_INT_RANGE, 1, G_MAXINT, NULL));
  } else {
    result_caps = gst_caps_new_empty_simple ("video/x-bayer");
    structure = gst_caps_get_structure (caps, 0);
    gst_caps_append_structure (result_caps,
  ...
  if (filter)
    result_caps =
        gst_caps_intersect_full (result_caps, filter, GST_CAPS_INTERSECT_FIRST);
  return result_caps;
}

static GstCaps *
gst_v4l2_bconvert_fixate_caps (GstV4l2BConvert * trans,
...
gst_v4l2_bconvert_prepare_output_buffer (GstV4l2BConvert * trans,
    GstBuffer * inbuf, GstBuffer ** outbuf)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (trans);
  GstBufferPool *pool = gst_v4l2_object_get_buffer_pool (self->v4l2output);
  GstFlowReturn ret = GST_FLOW_OK;
  GstBaseTransformClass *bclass = GST_V4L2_BCONVERT_CLASS (parent_class);
...
static GstFlowReturn
gst_v4l2_bconvert_bconvert (GstV4l2BConvert * trans, GstBuffer * inbuf,
    GstBuffer * outbuf)
{
  /* Nothing to do */
  return GST_FLOW_OK;
...
static gboolean
gst_v4l2_bconvert_sink_event (GstV4l2BConvert * trans, GstEvent * event)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (trans);
  gboolean ret;
...
gst_v4l2_bconvert_change_state (GstElement * element, GstStateChange transition)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (element);
  GstStateChangeReturn ret;

  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      if (!gst_v4l2_bconvert_open (self))
...
static void
gst_v4l2_bconvert_dispose (GObject * object)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (object);
...
static void
gst_v4l2_bconvert_finalize (GObject * object)
{
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (object);
...
static void
gst_v4l2_bconvert_init (GstV4l2BConvert * self)
{
  /* V4L2 object are created in subinstance_init */
  /* enable QoS */
  gst_base_transform_set_qos_enabled (GST_V4L2_BCONVERT (self), TRUE);
  self->import_buffer_alignment = DEFAULT_PROP_IMPORT_BUFFER_ALIG;
}

static void
gst_v4l2_bconvert_subinstance_init (GTypeInstance * instance, gpointer g_class)
{
  GstV4l2BConvertClass *klass = GST_V4L2_BCONVERT_CLASS (g_class);
  GstV4l2BConvert *self = GST_V4L2_BCONVERT (instance);
...
static void
gst_v4l2_bconvert_class_init (GstV4l2BConvertClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (gst_v4l2_bconvert_debug, "v4l2xm2msc", 0,
      "V4L2 Converter");

  gst_element_class_set_static_metadata (element_class,
      "ISP MIMO V4L2 Video Converter",
      "Filter/Converter/Video/Scaler",
      "Transform stream from bayer formats to raw formats and resolutions downscaling feature using ISP MIMO v4l2 m2m driver",
      "Nicolas Dufresne <nicolas.dufresne@collabora.com>,"
      "Tirupathi Korla <tirupath@amd.com>");

  gobject_class->dispose = GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_dispose);
  gobject_class->finalize = GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_finalize);
  gobject_class->set_property =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_set_property);
  gobject_class->get_property =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_get_property);

  base_transform_class->stop = GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_stop);
  base_transform_class->set_caps =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_set_caps);
  base_transform_class->query = GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_query);
  base_transform_class->sink_event =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_sink_event);
  base_transform_class->decide_allocation =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_decide_allocation);
  base_transform_class->propose_allocation =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_propose_allocation);
  base_transform_class->transform_caps =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_bconvert_caps);
  base_transform_class->fixate_caps =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_fixate_caps);
  base_transform_class->prepare_output_buffer =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_prepare_output_buffer);
  base_transform_class->transform =
      GST_DEBUG_FUNCPTR (gst_v4l2_bconvert_bconvert);
...
static void
gst_v4l2_bconvert_subclass_init (gpointer g_class, gpointer data)
{
  GstV4l2BConvertClass *klass = GST_V4L2_BCONVERT_CLASS (g_class);
  GstElementClass *element_class = GST_ELEMENT_CLASS (g_class);
  GstV4l2BConvertCData *cdata = data;
...
/* Probing functions */
gboolean
gst_v4l2_is_bconvert (GstCaps * sink_caps, GstCaps * src_caps)
{
  gboolean ret = FALSE;
...
void
gst_v4l2_bconvert_register (GstPlugin * plugin, const gchar * basename,
    const gchar * device_path, GstCaps * sink_caps, GstCaps * src_caps)
{
  ...
  GType type, subtype;
  gchar *type_name;
  GstV4l2BConvertCData *cdata;

  cdata = g_new0 (GstV4l2BConvertCData, 1);
  cdata->device = g_strdup (device_path);
  cdata->sink_caps = gst_caps_ref (sink_caps);
  cdata->src_caps = gst_caps_ref (src_caps);

  type = gst_v4l2_bconvert_get_type ();
  g_type_query (type, &type_query);
  memset (&type_info, 0, sizeof (type_info));
  type_info.class_size = type_query.class_size;
  type_info.instance_size = type_query.instance_size;
  type_info.class_init = gst_v4l2_bconvert_subclass_init;
